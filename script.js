...
function loadQuestion() {
  ...
  // Safety: if required elements don't exist, stop gracefully
  if (!questionBox || !questionText || answerButtons.length === 0) return;
...
function checkAnswer(selected) {
  if (selected === questions[currentQuestion].answer) {
    ...
    // ✅ Compute the accumulated money (exact amount before the wrong answer)
    let moneyWon = "₦0";
    if (currentQuestion === 0) {
      moneyWon = "₦0";
    } else {
      // last correct question index = currentQuestion - 1
      moneyWon = prizeMoney[currentQuestion - 1] || "₦0";
    }

    // Redirect to lost.html and pass the amount as a URL parameter
    window.location.href = `lost.html?money=${encodeURIComponent(moneyWon)}`;
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  // Safely link lifeline buttons if they exist
  const lifelineButtons = document.querySelectorAll(".lifelines button");
  if (lifelineButtons.length >= 1) lifelineButtons[0].onclick = use5050;
  if (lifelineButtons.length >= 2) lifelineButtons[1].onclick = usePhone;
  if (lifelineButtons.length >= 3) lifelineButtons[2].onclick = useAudience;

  // Load first question if quiz elements exist
  loadQuestion();
});
